<style>
  @import url(./trend.less);
</style>

<template>
  <div>
    <div class="trends" v-for="(item, index) in trendList" :key="index">
...
export default {
  ...
  methods: {
    _getlist () {
      getList({page: this.page, type: 1}).then((result) => {
        if (!result) return
        this.page = this.page+1
        this.trendList = result.data   // 20
      })
    },
    _Showdelect (index) {
      this.modal2 = true
      this.content = this.trendList[index]
    ...
    _delect () {
      delect({_id: this.content._id, type: 1}).then((result) => {
        if (!result) return
        this.$Message.success('删除成功')
        this.modal2 = false
        this.trendList.splice(this.content.i, 1)
      })
    },    
